#!/usr/bin/env node
// badname — the shared corpus of pathological file paths.
//
//   badname check [dir]        scan a tree for corpus hazards (read-only)
//   badname seed [dir]         materialize fixtures into an empty sandbox
//   badname roundtrip -- <cmd> run a shell command in a seeded sandbox
//   badname list               print corpus entries
//
// Run `badname help <command>` for details.
...
const HELP = {
  default: `badname — the shared corpus of pathological file paths

Usage:
  badname check [dir] [--skip name,...] [--json]
  badname seed [dir] [--category <id>] [--json]
  badname roundtrip -- <shell command> [--json]
  badname list [--category <id>] [--json]
  badname help [command]
  badname version
...
roundtrip  Seeds a temp sandbox, runs your shell command inside it, then
           ...
           Exits 1 on mangling. Example:
             badname roundtrip -- bash -c 'tar cf out.tar . && mkdir x && tar xf out.tar -C x && rm -rf out.tar'
list       Print corpus entries (visible form; invisible chars escaped).

Docs: https://github.com/sainzs/badname`,
}
...
async function main () {
  ...
  if (cmd === 'seed') {
    ...
    const dirArg = opts._[0] ?? join(tmpdir(), 'badname-sandbox')
  ...
  if (cmd === 'roundtrip') {
    ...
    if (dd === -1 || rest.length <= dd + 1) {
      console.error('usage: badname roundtrip -- <shell command>')
      process.exit(2)
    }
...
main().catch(err => {
  console.error('badname: ' + err.message)
  process.exit(1)
})
