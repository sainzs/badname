...
export function formatCheckReport ({ hits, scanned }, json) {
  ...
  if (hits.length === 0) {
    return `memex: clean — scanned ${scanned} entries, no corpus hazards found.`
  }
  const lines = []
  lines.push(`memex: ${hits.length} hazard(s) found among ${scanned} entries`)
  lines.push('')
  ...
  lines.push('')
  lines.push('Corpus: https://github.com/sainzs/memex — fixture docs in corpus/corpus.json')
  return lines.join('\n')
}
