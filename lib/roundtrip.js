// `memex roundtrip` — run a shell command inside a freshly seeded
...
export async function roundtrip (command, opts = {}) {
  ...
  const dir = await mkdtemp(join(tmpdir(), 'memex-rt-'))
  ...
  const filteredBefore = before.filter(n => !n.startsWith('memex-rt-'))
