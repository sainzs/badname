...
const BIN = join(ROOT, 'bin', 'badname.js')
...
test('check exits 0 on a clean tree and 1 on hazards', async () => {
  const clean = await mkdtemp(join(tmpdir(), 'badname-cli-'))
  assert.equal(run(['check', clean]).status, 0)

  const dirty = await mkdtemp(join(tmpdir(), 'badname-cli-'))
...
test('seed writes into a fresh directory and exits 0', async () => {
  const dir = await mkdtemp(join(tmpdir(), 'badname-cli-seed-'))
